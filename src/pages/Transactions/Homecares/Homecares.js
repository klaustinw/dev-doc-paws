...
const Homecares = () => {
  ...
  const {
    data: {
      docs: transactions, hasPrevPage, hasNextPage, page, totalDocs, pagingCounter, limit, totalPages
    },
    status,
    refetch
  } = useFetch(`${base_url}/admin/homecare?page=${currentPage}&limit=${currentLimit}`);
  ...
  const handleButton = (e, id, isApproved) => {
    ...
    async function main() {
      ...
      setSubmitting(true);
      await fetchPost("/admin/homecare/action", { id, action });
      setSubmitting(false);
      refetch();
    }
    main();
  ...
  return (
    <Row>
      <Col>
        <Card>
          <Card.Header>
            <Card.Title as="h5">Homecares List</Card.Title>
